...
if [ -z "$DOMAIN_DIR" ]; then
  ...
  echo "This might be a discovery repository - checking for discovery structure..."
  
  # Check if we have discovery structure
  if [ -d "discovery" ]; then
    DOMAIN_DIR="discovery"
    echo "✅ Found discovery domain directory: $DOMAIN_DIR"
  else
    echo "❌ No domain directory or discovery found"
    exit 1
  fi
else
